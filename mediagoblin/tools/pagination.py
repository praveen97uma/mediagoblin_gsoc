...
class Pagination(object):
    ...
    def __init__(self, page, cursor, per_page=PAGINATION_DEFAULT_PER_PAGE,
                 jump_to_id=False, extra_get_params=None):
        """
        ...
        Args:
         ...
         - extra_get_params: extra parameters to add to the URL of each page
           as generated by get_page_url_explicit
        """
        self.page = page
        ...
        self.active_id = None
        self.extra_get_params = extra_get_params or {}

        self.cursor_iter = getattr(cursor, "cursor_iter", cursor)

        if jump_to_id:
            cursor = copy.copy(self.cursor_iter)

            for (doc, increment) in izip(self.cursor_iter, count(0)):
    ...
    def get_page_url_explicit(self, base_url, get_params, page_no):
        ...
        new_get_params.update(self.extra_get_params)
        return "%s?%s" % (
